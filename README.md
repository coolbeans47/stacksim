...
| AWS X-Ray | Official `PutTraceSegments`, trace-summary/batch reads, service/trace graphs, parent-based Default sampling, durable encrypted regional traces, API Gateway stage segments, Lambda context propagation, IAM, and the trace console | Custom sampling rules/groups, Lambda-owned automatic segments, daemon/OTLP ingestion, broader producers, Insights, Transaction Search, X-Ray CloudFormation resources, and the remaining action catalog |
