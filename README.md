...
| Amazon DynamoDB | All 57 DynamoDB API actions and all four Streams actions: table, item, and batch CRUD; expressions; LSI/GSI indexes; evaluated-item pagination; transactions; TTL; optional deterministic capacity enforcement and execution-derived consumed-capacity metrics; tags and resource policies; backups and point-in-time recovery; durable Streams and Lambda polling; DynamoDB-subset PartiQL with exact-get/query/partition-`IN`/scan planning, statement-derived IAM, and singleton/batch/transaction tooling; MREC global tables; contributor insights; Kinesis destination configuration; and local-bucket DynamoDB JSON/GZIP import/export through an S3-owned transfer port (plus optional `file://` with `STACKSIM_ALLOW_LOCAL_FILES`) | DAX (a separate service); real Kinesis record delivery; Ion, CSV, ZSTD, and incremental transfers; KMS export encryption; MRSC, witnesses, multi-account groups, and replica KMS keys; generic SQL and unsupported PartiQL such as joins, subqueries, aggregates, identifier parameters, multiple statements, and statement-level `LIMIT`; production stream shard splitting/merging; an automatic scaling controller; and AWS distributed throughput or infrastructure |
| AWS Lambda | Local Node.js ZIP and OCI-image functions, versions, aliases, layers, URLs, concurrency, async invocation, and DynamoDB Streams/SQS event sources. Node ZIP execution environments are bounded and warm-reused, including module state, client singletons, private `/tmp`, and truthful initialized provisioned concurrency. REPORT distinguishes configured memory from worker lifetime peak RSS or unavailable usage. | ZIP memory/scratch quota enforcement; container peak-memory measurement; non-Node managed runtimes, remote ECR publishing, reusable/prewarmed image containers, image-function provisioned concurrency, and AWS-managed runtime, VPC, or compute infrastructure |
| AWS Step Functions | Durable Standard JSONPath Workflows with Lambda, DynamoDB item, SQS send/callback, SNS publish/callback, EventBridge put-events, and nested workflow request/`.sync`/callback integrations; stable task-attempt identity plus owning-service acceptance receipts; Activities and all task-token APIs; retry/catch; restart-safe Parallel, Inline Map, callbacks and workers; execution-role IAM; idempotently correlated EventBridge/Scheduler starts; metrics/events; console inspection; and CDK/CloudFormation StateMachine and Activity deployment with authorized pinned S3 definitions, substitutions, ownership and recoverable lifecycle. Receipt payloads remain in private owning stores and terminal receipts are released after workflow completion is durable. | Express, JSONata, enabled execution logging, versions/aliases/redrive, Distributed Map, AWS SDK integrations, HTTP tasks, cross-account tasks, unlisted optimized integrations, and KMS/X-Ray. An arbitrary synchronous Lambda (or other non-idempotent external side effect) that stops after dispatch but before its owning completion receipt remains explicitly ambiguous and is never retried blindly. |
| Amazon API Gateway | REST, HTTP, and WebSocket APIs; Lambda, DynamoDB, SQS, HTTP, and selected private integrations; auth, stages, domains, logging, metrics, validation, durable response caching, and real REST-stage X-Ray tracing | Every AWS service integration, real VPC networking, edge infrastructure, managed certificates, and AWS KMS-backed cache encryption |
| AWS X-Ray | Official `PutTraceSegments`, trace-summary/batch reads, service/trace graphs with summed completed durations, parent-based Default sampling, durable encrypted regional traces, API Gateway stage segments, Lambda context propagation, IAM, and the trace console | Custom sampling rules/groups, Lambda-owned automatic segments, daemon/OTLP ingestion, broader producers, Insights, Transaction Search, X-Ray CloudFormation resources, and the remaining action catalog |
| AWS AppSync | API-key GraphQL APIs, schemas, `NONE` and DynamoDB data sources, VTL unit and pipeline resolvers, queries/mutations, `AWS_IAM` and local Cognito access-token request/field authorization, the frozen public and owner Todo CRUD/subscription graphs, and console exploration | JavaScript resolvers, Lambda/OIDC authorization, group-rule breadth, general enhanced subscription filters/invalidation, Events, caches, merged APIs, custom domains, and the complete AppSync catalog |
| AWS Amplify Gen 2 (exact pinned fixtures) | Unmodified `ampx` sandbox deployment and CLI-written outputs; public Todo workflows; email Auth/SRP; private owner CRUD/pagination/subscriptions; separate authenticated Identity Pool/IAM Data; Auth update/rollback/restart/delete and two sandboxes. See the [capability closeout](docs/gap-3-authenticated-app-closeout.md). | Unrestricted compatibility; other package versions or backend graphs; general AMX-M3 crash recovery; Gen 1; social/group/advanced Auth breadth; Storage, user Functions, relationships, indexes, custom operations, auxiliary generation commands, `pipeline-deploy`, and Hosting |
...
| Amazon CloudFront (CFR-01 subset) | Account-global SDK and CloudFormation lifecycle for Distribution, Function, Origin Access Control, and Response Headers Policy resources; private same-account S3 origins with enforced OAC policy context; isolated DEVELOPMENT/LIVE viewer-request Functions; managed caching, compression, security headers, invalidation; canonical AWS-shaped domain outputs; and a separate per-distribution local HTTPS viewer | Public/custom origins, OAI, aliases/custom certificates, WAF, logging, Lambda@Edge, runtime 2.0/KVS, customer cache policies, advanced distribution resources, and the remaining CloudFront catalog |
| Amazon SQS | Standard, fair, and FIFO queues; message I/O; DLQs and durable start/list/cancel redrive tasks; policies, SSE-SQS, Lambda triggers and console recovery workflow. [23-action inventory](docs/sqs-action-inventory.md). | KMS, VPC endpoints, provisioned Lambda pollers and production throughput; full SQS-05 remains open |
...
| AWS IAM and STS | Users, groups, access keys, roles, policies, exact path-qualified authorization targets, bounded policy/statement explanations, SigV4 enforcement, `AssumeRole`, session policies/tags, and caller identity | IAM Identity Center, Organizations, and the complete IAM/STS administration surface |
...
| Amazon Cognito Identity Pools | Enhanced `GetId` / `GetCredentialsForIdentity`, User Pool ID-token login, atomic guest linking, generated no-group `Token`/`AuthenticatedRole` mapping, IAM control APIs, and two CloudFormation types | Classic flow, other role mappings, role-bearing/group tokens in that mapping, principal tags, social IdPs, developer identities, public `AssumeRoleWithWebIdentity`, and the remaining Identity action catalog |
...
| --- | --- | --- |
| [Terraform HTTP API and DynamoDB](examples/terraform-api-dynamodb/README.md) | Uses the standard HashiCorp AWS provider to deploy a Node.js Lambda, API Gateway HTTP API, DynamoDB table, IAM role and policy, and CloudWatch log group directly to StackSim. | Terraform provider configuration, dependency graphs and local state, AWS-compatible endpoints, serverless API composition, smoke testing, and clean teardown without CloudFormation or an AWS account. |
| [Authenticated notes](examples/amplify-auth-notes/README.md) | Pinned Amplify Auth + Data with two users, private records, subscriptions, and a separate Identity Pool/IAM example. | User Pool identity, server-enforced ownership, temporary AWS credentials, intentional denials, sign-out, and repeatable sandbox cleanup. |
...
| [Step Functions deployment lifecycle](examples/cdk-step-functions-lifecycle/README.md) | Deploys common service integrations, an S3 definition with substitutions, and an Activity worker through pinned CDK; executes, updates, rolls back, retains, destroys and redeploys. | Deployment-role versus workflow-role authority, immutable definitions, actual downstream effects, ownership and cleanup. |
...
| [SQS recovery lab](examples/sqs-recovery-lab/README.md) | Breaks a Lambda consumer, inspects retries and DLQ messages, fixes code, redrives, cancels/restarts and verifies a durable result ledger. | Scenario H, scoped IAM, partial batches, at-least-once delivery, stable application keys and repeatable reset. |
...
| [X-Ray](docs/xray-console-guide.md) | Trace list, service map, timeline, safe segment view, sampling behavior, and repository diagnostics |
