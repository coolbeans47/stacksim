...
import { definitionScopes, eventDetails, executionPresentation, integrationReferences, lambdaReferences, parseStateMachineDefinition, payloadField } from "./step-functions-model.js";
...
function definitionGraph(definition, { activeName, selectedName, selectable = false } = {}) {
  ...
  return `<div class="sfn-nested-graphs">${scopes.map(scope => `<section class="sfn-graph-scope" aria-label="${escapeHtml(scope.label)}"><div class="sfn-scope-header"><h3>${escapeHtml(scope.label)}</h3><span class="muted">Starts at ${escapeHtml(scope.startAt ?? "–")}</span></div><div class="sfn-graph">${scope.states.map(({ name, state }) => {
    const content = `<strong>${escapeHtml(name)}</strong><span>${escapeHtml(state.Type)}</span><small>${escapeHtml(state.Next ?? (state.End || ["Succeed", "Fail"].includes(state.Type) ? "End" : state.Default ?? ""))}</small>${state.Resource ? `<small class="mono sfn-wrap">${escapeHtml(state.Resource)}</small>` : ""}${state.Retry?.length ? `<small>${state.Retry.length} retry polic${state.Retry.length === 1 ? "y" : "ies"}</small>` : ""}${state.Catch?.length ? `<small>${state.Catch.length} catcher${state.Catch.length === 1 ? "" : "s"}</small>` : ""}`;
...
function stateTable(model) {
  const rows = model.scopes.flatMap(scope => scope.states.map(item => {
    ...
    const value = model.history.active?.stateName === item.name ? "RUNNING" : failed ? "FAILED" : exited ? "SUCCEEDED" : related.length ? "ENTERED" : "NOT STARTED";
