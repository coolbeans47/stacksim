...
export const metadata = {
  ...
  search: [
    "cognito", "identity", "identity pool", "federated identities", "user pool", "app client",
    "users", "sign in", "sign up", "authentication", "jwt", "jwks", "getid",
  ],
};
...
const cognitoPanelHelp = {
  userPools: {
    ...
    support: "User-pool creation, users, groups, password and SRP authentication, recovery, MFA, Lambda triggers, signed JWTs, and local JWKS are active and persist locally. Temporary AWS credentials are issued by Identity pools in this Cognito console.",
  },
  poolDetails: {
    level: "Supported locally",
...
async function landing(context) {
  const [{ userPools }, { identityPools }] = await Promise.all([
    request("/_stacksim/api/cognito/user-pools"),
    request("/_stacksim/api/cognito-identity/identity-pools"),
  ]);
  ...
  context.main.innerHTML = `<div class="page-width cognito-page">${pageHeader("Cognito", "Create local user directories, administer users and groups, confirm users through the SES Inbox, and exchange User Pool ID tokens for temporary AWS credentials.", '<button class="button primary" data-action="create-user-pool">Create user pool</button>')}
    <div class="alert info"><strong>User pools development profile</strong><br>Supports self-service and administrator-created users, recovery, password and SRP authentication, refresh rotation, groups, TOTP/email MFA, Lambda triggers, signed tokens, and local JWKS. Temporary AWS credentials are issued by <a href="#/cognito-identity/identity-pools">Identity pools</a>.</div>
    <div class="cognito-summary-grid cognito-product-grid">
      ...
      <section class="card"><div class="card-header"><h2>Identity pools</h2></div><div class="card-body"><div class="metric">${identityPools.length}</div><p class="muted">Enhanced-flow credential brokers</p><a href="#/cognito-identity/identity-pools">View identity pools</a></div></section>
    </div>
    <div class="cognito-summary-grid cognito-product-grid">
