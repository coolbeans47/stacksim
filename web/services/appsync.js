...
function unsupportedBoundary() {
  return `<div class="alert info"><strong>Implemented boundary</strong><br>API-key, IAM and local Cognito access-token GraphQL authorization, generated VTL pipelines, NONE/DynamoDB data sources, and the frozen Amplify Todo realtime subscriptions are active. Lambda/OIDC authorization, general enhanced filters/invalidation, APPSYNC_JS, Lambda/HTTP/EventBridge data sources, caches, domains, merged APIs, AppSync Events, and unpinned Amplify graphs remain unavailable. The Auth learning fixtures use generated owner functions and local Identity Pool credentials.</div>`;
}

function createApiForm() {
  return `<form id="appsync-create-api"><div class="field"><label>API name</label><input name="name" required maxlength="65536" autocomplete="off" placeholder="local-notes"></div><div class="field"><label>Authorization mode</label><select name="authenticationType"><option value="API_KEY">API key</option></select><span class="hint">This form creates API-key APIs. Use the SDK or pinned Auth fixtures to configure Cognito and IAM modes.</span></div><div class="field"><label>Introspection</label><select name="introspectionConfig"><option value="ENABLED">Enabled</option><option value="DISABLED">Disabled</option></select></div><div class="field"><label>Owner contact (optional)</label><input name="ownerContact" maxlength="256"></div><div class="field"><label>Tags (JSON object)</label><textarea name="tags" maxlength="16384">{}</textarea></div><div class="actions"><a class="button" href="#/appsync/apis">Cancel</a><button class="button primary" type="submit">Create API</button></div></form>`;
}
