...
async function detailPage(name) {
  ...
  document.querySelector('[data-action="edit"]')?.addEventListener("click", () => context.showModal("Edit parameter value", `<div class="field"><label>New value</label><textarea name="value" required autocomplete="off"></textarea></div><div class="field"><label>Description</label><textarea name="description">${escapeHtml(described.Description ?? "")}</textarea></div><div class="field"><label>Allowed pattern</label><input name="allowedPattern" value="${escapeHtml(described.AllowedPattern ?? "")}"></div>`, "Save new version", async data => {
    const value = String(data.get("value"));
    await ssm("PutParameter", { Name: name, Type: described.Type, Value: value, Overwrite: true, Description: String(data.get("description") || ""), AllowedPattern: String(data.get("allowedPattern") || "") || undefined });
