...
function bindPolicyEditor(root) {
  ...
  const invalidate = () => { root.querySelector("[data-policy-validation-status]").textContent = "Not validated"; root.querySelector("[data-policy-summary]").innerHTML = ""; };
  root.querySelectorAll('[name="document"], [data-policy-effect], [data-policy-sid], [data-policy-actions], [data-policy-resources]').forEach(editor => editor.addEventListener("input", invalidate));
  root.querySelector("[data-policy-validate]").addEventListener("click", async () => {
    const status = root.querySelector("[data-policy-validation-status]"); const summary = root.querySelector("[data-policy-summary]");
    try {
      if (root.querySelector('[data-policy-editor-tab="visual"]').getAttribute("aria-selected") === "true") syncVisualToJson(root);
      const report = await rest("/_stacksim/api/iam/policy-validation", "POST", { Kind: "identity", PolicyDocument: JSON.parse(root.querySelector('[name="document"]').value) });
      status.textContent = report.valid ? "Valid policy" : "Validation failed";
      policyEditorError(root, report.errors.join(" "));
      const warnings = report.warnings.length ? `<div class="alert warning">${report.warnings.map(escapeHtml).join(" ")}</div>` : "";
      const rows = report.summary.map(item => `<tr><td>${escapeHtml(item.effect)}</td><td>${escapeHtml(item.service)}</td><td class="mono">${escapeHtml(item.actions.join(", "))}</td><td class="mono">${escapeHtml(item.resources.join(", "))}</td><td>${escapeHtml(item.conditions.join(", ") || "None")}</td></tr>`).join("");
      summary.innerHTML = `${warnings}${rows ? `<h3>Permission summary</h3><div class="table-wrap"><table><thead><tr><th>Effect</th><th>Service</th><th>Actions</th><th>Resources</th><th>Conditions</th></tr></thead><tbody>${rows}</tbody></table></div>` : ""}`;
    } catch (error) { status.textContent = "Validation failed"; summary.innerHTML = ""; policyEditorError(root, error instanceof Error ? error.message : String(error)); }
  });
}
...
function policyExplanation(item) {
  if (item.provenanceUnavailable) return '<p class="muted">Policy provenance unavailable for this historical decision.</p>';
  const entries = item.provenance ?? [];
  const sourceLabel = source => [source.kind, source.policyArn, source.versionId ? `version ${source.versionId}` : "", source.entityType, source.entityArn, source.service, source.resourceArn, source.policyName, source.revision ? `revision ${source.revision}` : ""].filter(Boolean).join(" · ");
  return `<details class="iam-policy-explanation"><summary>Policy explanation (${item.provenanceMatchedCount ?? 0} matched / ${item.provenanceTotalCount ?? 0} evaluated)</summary><p class="muted small">Statement indexes start at 0. Identity and resource grants combine with applicable boundary and session restrictions. The decision reason above describes the final result.</p>${item.diagnosticFieldsTruncated?.length ? `<p class="alert warning">Oversized diagnostic fields truncated: ${item.diagnosticFieldsTruncated.map(escapeHtml).join(", ")}.</p>` : ""}${item.provenanceTruncated ? `<p class="alert warning">Provenance truncated: showing ${entries.length} of ${item.provenanceTotalCount} entries (32 entries / 16 KiB maximum).</p>` : ""}${entries.length ? `<ul>${entries.map(entry => `<li><strong>${escapeHtml(entry.layer)} · ${escapeHtml(entry.effect ?? entry.status)} · ${entry.matched ? "Matched" : "Not matched"}</strong><p class="mono" style="overflow-wrap:anywhere">${escapeHtml(sourceLabel(entry.source))}</p><p>${entry.statementIndex === undefined ? escapeHtml(entry.status) : `Statement index ${entry.statementIndex}${entry.sid === undefined ? "" : ` · SID ${escapeHtml(entry.sid)}`}`} · ${escapeHtml(entry.actionScope ?? "")} / ${escapeHtml(entry.resourceScope ?? "")}</p></li>`).join("")}</ul>` : '<p class="muted">No policy statements were available to contribute to this decision. See the decision reason.</p>'}</details>`;
}
...
async function authorizationDecisions(context) {
  ...
  main.innerHTML = `<div class="page-width">${pageHeader("Authorization decisions", "Local diagnostic history. This page is not part of the IAM console.")}<div class="alert info"><strong>Local simulator tooling</strong><br>Expand a policy explanation to identify the exact policy, revision and statement behind a decision.</div><div class="card"><div class="table-wrap">${decisions.length ? `<table class="iam-decisions-table" style="table-layout:fixed;width:100%"><thead><tr><th>Time</th><th>Decision</th><th>Principal</th><th>Action</th><th>Resource</th><th>Reason</th></tr></thead>${decisions.map(item => `<tbody class="iam-decision"><tr><td>${formatDate(item.time)}</td><td><span class="status ${item.decision === "allowed" ? "" : "error"}">${escapeHtml(item.decision)}</span></td><td class="mono" style="overflow-wrap:anywhere;white-space:normal">${escapeHtml(item.principalArn)}</td><td style="overflow-wrap:anywhere">${escapeHtml(item.action)}</td><td class="mono" style="overflow-wrap:anywhere;white-space:normal">${escapeHtml(item.resource)}</td><td>${escapeHtml(item.reason)}</td></tr><tr><td colspan="6">${policyExplanation(item)}</td></tr></tbody>`).join("")}</table>` : emptyState("◇", "No authorization decisions", "Enable enforce mode and make signed SDK requests to populate this diagnostic history.")}</div></div></div>`;
