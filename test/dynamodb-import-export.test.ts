...
async function waitForImport(client: DynamoDBClient, importArn: string, clock: TestClock): Promise<any> {
  ...
  throw new Error(`import ${importArn} did not leave IN_PROGRESS`);
}
...
test("DynamoDB local file export/import round trips DynamoDB JSON with durable job catalogs", async () => {
  ...
  try {
    ...
    await assert.rejects(client.send(new ExportTableToPointInTimeCommand({ TableArn: source.TableDescription!.TableArn!, S3Bucket: "missing-real-bucket" })), (error: any) => error.name === "NoSuchBucket" || error.name === "ValidationException");
    await assert.rejects(client.send(new ExportTableToPointInTimeCommand({ TableArn: source.TableDescription!.TableArn!, S3Bucket: bucketUrl, ExportFormat: "ION" })), (error: any) => error.name === "ValidationException" && /codec-blocked/.test(error.message));
    ...
    assert.equal((await client.send(new ListExportsCommand({ TableArn: source.TableDescription!.TableArn!, MaxResults: 1 }))).ExportSummaries?.[0].ExportArn, exportArn); const completed = await waitForExport(client, exportArn, clock); assert.equal(completed.ExportStatus, "COMPLETED"); assert.equal(completed.ItemCount, 2);
    ...
    const creation = { TableName: "TransferImported", BillingMode: "PAY_PER_REQUEST", AttributeDefinitions: [{ AttributeName: "id", AttributeType: "S" }, { AttributeName: "category", AttributeType: "S" }], KeySchema: [{ AttributeName: "id", KeyType: "HASH" }], GlobalSecondaryIndexes: [{ IndexName: "ByCategory", KeySchema: [{ AttributeName: "category", KeyType: "HASH" }], Projection: { ProjectionType: "ALL" } }] } as any; const imported = await client.send(new ImportTableCommand({ S3BucketSource: { S3Bucket: bucketUrl, S3KeyPrefix: `roundtrip/AWSDynamoDB/${exportId}/data` }, InputFormat: "DYNAMODB_JSON", InputCompressionType: "GZIP", TableCreationParameters: creation, ClientToken: "import-roundtrip" })); const importArn = imported.ImportTableDescription!.ImportArn!; assert.equal(imported.ImportTableDescription?.ImportStatus, "IN_PROGRESS"); assert.equal((await client.send(new ImportTableCommand({ S3BucketSource: { S3Bucket: bucketUrl, S3KeyPrefix: `roundtrip/AWSDynamoDB/${exportId}/data` }, InputFormat: "DYNAMODB_JSON", InputCompressionType: "GZIP", TableCreationParameters: creation, ClientToken: "import-roundtrip" }))).ImportTableDescription?.ImportArn, importArn); const importDescription = await waitForImport(client, importArn, clock); assert.equal(importDescription.ImportStatus, "COMPLETED"); assert.equal(importDescription.ImportedItemCount, 2); assert.equal((await client.send(new GetItemCommand({ TableName: "TransferImported", Key: { id: { S: "alpha" } } }))).Item?.count?.N, "1"); assert.equal((await client.send(new QueryCommand({ TableName: "TransferImported", IndexName: "ByCategory", KeyConditionExpression: "category = :category", ExpressionAttributeValues: { ":category": { S: "music" } } }))).Count, 1); assert.equal((await client.send(new ListImportsCommand({ TableArn: importDescription.TableArn!, PageSize: 1 }))).ImportSummaryList?.[0].ImportArn, importArn);
