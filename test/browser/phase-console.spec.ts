...
test.describe("target phase console workflows", () => {
  ...
  test("DDB-07 creates, inspects, restores, and deletes backups and restores the latest PITR state", async ({ page }) => {
    const dynamodb = new DynamoDBClient(sdkOptions(simulator));
    try {
      await dynamodb.send(new CreateTableCommand({ TableName: "BrowserBackups", BillingMode: "PAY_PER_REQUEST", AttributeDefinitions: [{ AttributeName: "id", AttributeType: "S" }], KeySchema: [{ AttributeName: "id", KeyType: "HASH" }] })); await dynamodb.send(new PutItemCommand({ TableName: "BrowserBackups", Item: { id: { S: "record" }, version: { N: "1" } } }));
  ...
  test("LAM-05 creates and manages a DynamoDB stream trigger from both service consoles", async ({ page }) => {
    ...
    try {
      await dynamodb.send(new CreateTableCommand({ TableName: "BrowserStreams", BillingMode: "PAY_PER_REQUEST", AttributeDefinitions: [{ AttributeName: "id", AttributeType: "S" }], KeySchema: [{ AttributeName: "id", KeyType: "HASH" }] })); const errors = browserErrors(page); await page.goto(`${consoleUrl}#/dynamodb/tables/BrowserStreams/streams`); const streamCard = page.locator(".stream-card"); const triggerCard = page.locator(".trigger-card");
  ...
  test("DDB-09 validates, revisions, warns about, saves, and deletes a table resource policy", async ({ page }) => {
    const dynamodb = new DynamoDBClient(sdkOptions(simulator));
    try {
      await dynamodb.send(new CreateTableCommand({ TableName: "BrowserPermissions", BillingMode: "PAY_PER_REQUEST", AttributeDefinitions: [{ AttributeName: "id", AttributeType: "S" }], KeySchema: [{ AttributeName: "id", KeyType: "HASH" }] })); const tableArn = "arn:aws:dynamodb:eu-west-1:000000000000:table/BrowserPermissions"; const errors = browserErrors(page);
  ...
  test("DDB-10 creates, backfills, lists, replicates through, and removes a global table replica", async ({ page }) => {
    ...
    try {
      await west.send(new CreateTableCommand({ TableName: "BrowserGlobal", BillingMode: "PAY_PER_REQUEST", AttributeDefinitions: [{ AttributeName: "id", AttributeType: "S" }], KeySchema: [{ AttributeName: "id", KeyType: "HASH" }] })); await west.send(new PutItemCommand({ TableName: "BrowserGlobal", Item: { id: { S: "backfill" }, value: { S: "before" } } })); const errors = browserErrors(page);
  ...
  test("DDB-10 exports and imports DynamoDB JSON through the opted-in local file workflow", async ({ page }) => {
    ...
    try {
      await dynamodb.send(new CreateTableCommand({ TableName: "BrowserTransfer", BillingMode: "PAY_PER_REQUEST", AttributeDefinitions: [{ AttributeName: "id", AttributeType: "S" }], KeySchema: [{ AttributeName: "id", KeyType: "HASH" }] })); await dynamodb.send(new PutItemCommand({ TableName: "BrowserTransfer", Item: { id: { S: "roundtrip" }, value: { S: "browser" } } })); await dynamodb.send(new UpdateContinuousBackupsCommand({ TableName: "BrowserTransfer", PointInTimeRecoverySpecification: { PointInTimeRecoveryEnabled: true } })); const errors = browserErrors(page);
      await page.goto(`${consoleUrl}#/dynamodb/tables/BrowserTransfer/streams`); const exportCard = page.locator(".export-card"); await expect(exportCard.getByText("No exports", { exact: true })).toBeVisible(); await exportCard.getByRole("button", { name: "Export to local files" }).click(); let dialog = page.getByRole("dialog"); await expect(dialog.getByText("Local simulator extension", { exact: true })).toBeVisible(); await dialog.getByLabel("Local bucket location").fill(bucket); await dialog.getByLabel("Export prefix").fill("browser-roundtrip"); await dialog.getByRole("button", { name: "Export", exact: true }).click(); await expect(exportCard.getByText("COMPLETED", { exact: true })).toBeVisible(); await expect(exportCard.getByText("BrowserTransfer", { exact: true })).toBeVisible();
      const exports = await dynamodb.send(new ListExportsCommand({ TableArn: "arn:aws:dynamodb:eu-west-1:000000000000:table/BrowserTransfer" })); const exportId = exports.ExportSummaries![0].ExportArn!.split("/export/")[1]; await page.goto(`${consoleUrl}#/dynamodb/imports`); await expect(page.getByText("Local simulator behavior", { exact: true })).toBeVisible(); await page.getByRole("button", { name: "Import from local files" }).click(); dialog = page.getByRole("dialog"); await dialog.getByLabel("Local bucket location").fill(bucket); await dialog.getByLabel("Key prefix or file").fill(`browser-roundtrip/AWSDynamoDB/${exportId}/data`); await dialog.getByLabel("Table name").fill("BrowserImported"); await dialog.getByLabel("Partition key").fill("id"); await dialog.getByRole("button", { name: "Import table" }).click(); const row = page.getByRole("row").filter({ hasText: "BrowserImported" }); await expect(row.getByText("COMPLETED", { exact: true })).toBeVisible(); expect((await dynamodb.send(new GetItemCommand({ TableName: "BrowserImported", Key: { id: { S: "roundtrip" } } }))).Item?.value?.S).toBe("browser");
      await page.goto(`${consoleUrl}#/dynamodb/exports`); await expect(page.getByRole("heading", { name: "Exports and streams", exact: true })).toBeVisible(); await expect(page.getByText("Local file extension enabled", { exact: true })).toBeVisible(); await page.setViewportSize({ width: 390, height: 844 }); await expect(page.getByRole("cell", { name: "BrowserTransfer" })).toBeVisible(); expect(errors).toEqual([]);
    } finally { dynamodb.destroy(); }
  });
  ...
  test("DDB-10 configures contributor insights and explores local hot-key metrics", async ({ page }) => {
    const dynamodb = new DynamoDBClient(sdkOptions(simulator));
    try {
      await dynamodb.send(new CreateTableCommand({ TableName: "BrowserContributors", BillingMode: "PAY_PER_REQUEST", AttributeDefinitions: [{ AttributeName: "id", AttributeType: "S" }, { AttributeName: "category", AttributeType: "S" }], KeySchema: [{ AttributeName: "id", KeyType: "HASH" }], GlobalSecondaryIndexes: [{ IndexName: "ByCategory", KeySchema: [{ AttributeName: "category", KeyType: "HASH" }], Projection: { ProjectionType: "ALL" } }] })); const errors = browserErrors(page);
  ...
  test("DDB-10 manages a configuration-only Kinesis streaming destination", async ({ page }) => {
    ...
    try {
      await dynamodb.send(new CreateTableCommand({ TableName: "BrowserKinesis", BillingMode: "PAY_PER_REQUEST", AttributeDefinitions: [{ AttributeName: "id", AttributeType: "S" }], KeySchema: [{ AttributeName: "id", KeyType: "HASH" }] })); const errors = browserErrors(page);
  ...
  test("CW-04 explores an empty metric catalog, filters and graphs metrics, and exposes source queries", async ({ page }) => {
    ...
    try {
      ...
      await dynamodb.send(new CreateTableCommand({ TableName: "PhaseMetricTable", BillingMode: "PAY_PER_REQUEST", AttributeDefinitions: [{ AttributeName: "id", AttributeType: "S" }], KeySchema: [{ AttributeName: "id", KeyType: "HASH" }] })); await dynamodb.send(new PutItemCommand({ TableName: "PhaseMetricTable", Item: { id: { S: "browser" } } })); await page.goto(`${consoleUrl}#/dynamodb/tables/PhaseMetricTable/monitor`); await expect(page.getByRole("heading", { name: "Table metrics" })).toBeVisible(); await expect(page.getByLabel("PhaseMetricTable DynamoDB").getByText("Consumed write capacity", { exact: true })).toBeVisible(); expect(errors).toEqual([]);
