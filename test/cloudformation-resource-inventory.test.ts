...
test("the public server registry contains exactly the 112 production providers", async () => {
  ...
  try {
    ...
    assert.equal(CLOUDFORMATION_RESOURCE_INVENTORY.length, 112);
    ...
    assert.equal(new Set(providers.map(provider => provider.typeName)).size, 112);
