...
test("the public server registry contains exactly the 111 production providers", async () => {
  ...
  try {
    ...
    assert.equal(CLOUDFORMATION_RESOURCE_INVENTORY.length, 111);
    ...
    assert.equal(new Set(providers.map(provider => provider.typeName)).size, 111);
