...
test("the public server registry contains exactly the 109 production providers", async () => {
  ...
  try {
    ...
    assert.equal(CLOUDFORMATION_RESOURCE_INVENTORY.length, 109);
    ...
    assert.equal(new Set(providers.map(provider => provider.typeName)).size, 109);
