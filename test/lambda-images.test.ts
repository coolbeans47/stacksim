...
test("Lambda Docker images use inspect-only resolution and an isolated Runtime API container with timeout cleanup", async () => {
  ...
  try {
    ...
    const invoked = await lambda.send(new InvokeCommand({ FunctionName: "local-docker-image", LogType: "Tail", Payload: Buffer.from('{"hello":"image"}') })); assert.equal(invoked.FunctionError, undefined); assert.deepEqual(JSON.parse(Buffer.from(invoked.Payload ?? []).toString("utf8")), { from: "container", event: { hello: "image" } }); assert.equal(runtimeFailure, undefined); assert.match(Buffer.from(invoked.LogResult!, "base64").toString(), /Max Memory Used: unavailable \(container-peak-unavailable\)/);
    assert.equal(containerConfig.Image, `${imageUri.slice(0, imageUri.lastIndexOf(":"))}@${imageDigest}`); assert.equal(containerConfig.User, "1000:1000"); assert.deepEqual(containerConfig.Cmd, ["app.handler"]); assert.equal(containerConfig.HostConfig.ReadonlyRootfs, true); assert.equal(containerConfig.HostConfig.Memory, 128 * 1024 * 1024); assert.deepEqual(containerConfig.HostConfig.CapDrop, ["ALL"]); assert.deepEqual(containerConfig.HostConfig.SecurityOpt, ["no-new-privileges:true"]); assert.equal(containerConfig.HostConfig.PidsLimit, 128); assert.match(containerConfig.HostConfig.NetworkMode, /^stacksim-lambda-/); assert.match(containerConfig.HostConfig.Tmpfs["/tmp"], /size=512m/); assert.ok(!requests.some(request => request.includes("/images/create")), "neither resolution nor invocation may pull an image");
    await lambda.send(new UpdateFunctionConfigurationCommand({ FunctionName: "local-docker-image", Timeout: 1, LoggingConfig: { LogFormat: "JSON" } })); runtimeMode = "timeout"; const timedOut = await lambda.send(new InvokeCommand({ FunctionName: "local-docker-image", LogType: "Tail", Payload: Buffer.from("{}") })); assert.equal(timedOut.FunctionError, "Unhandled"); const report = Buffer.from(timedOut.LogResult!, "base64").toString().trim().split("\n").map(line => JSON.parse(line)).find(line => line.type === "platform.report").record; assert.equal(report.metrics.maxMemoryUsedMB, undefined); assert.equal(report.memoryUsage.reason, "container-peak-unavailable"); assert.equal(JSON.parse(Buffer.from(timedOut.Payload ?? []).toString("utf8")).errorType, "TimeoutError"); assert.ok(requests.filter(request => request.startsWith("DELETE /v1.41/containers/container-id")).length >= 2); assert.ok(requests.filter(request => request === "DELETE /v1.41/networks/network-id").length >= 2);
  } finally {
