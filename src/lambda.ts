import { trustPolicySource, resourcePolicySource, mergeProvenance } from "./iam/provenance.js";
...
export interface InvokeResult { memoryUsage?: LambdaMemoryUsage; payload: Buffer; functionError?: string; statusCode: number; logResult?: string; requestId: string; durationMs: number; billedDurationMs: number; executedVersion: string; durableExecutionArn?: string; /** Internal: the simulator stopped while this invocation was active. */ interrupted?: boolean }
...
export class LambdaService {
  ...
  private validateRole(roleArn: string, principal?: PrincipalContext, associatedResourceArn?: string): void {
    ...
    if (evaluateTrust(role.assumeRolePolicyDocument, "lambda.amazonaws.com", "sts:AssumeRole", { "aws:PrincipalServiceName": "lambda.amazonaws.com" }, trustPolicySource(role)).decision !== "allowed") throw new AwsError("InvalidParameterValueException", "The role defined for the function cannot be assumed by Lambda");
  ...
  private async runtimeCredentials(roleArn: string, lineage: string[]): Promise<Record<string, string>> {
    ...
    if (evaluateTrust(role.assumeRolePolicyDocument, "lambda.amazonaws.com", "sts:AssumeRole", { "aws:PrincipalServiceName": "lambda.amazonaws.com" }, trustPolicySource(role)).decision !== "allowed") throw new AwsError("InvalidParameterValueException", `Lambda execution role ${roleArn} no longer trusts lambda.amazonaws.com`, 400);
  ...
  functionUrlResourcePolicy(principal: string | Pick<PrincipalContext, "principalArn" | "roleArn">, target: FunctionUrlTarget, action: "lambda:InvokeFunction" | "lambda:InvokeFunctionUrl", context: Record<string, unknown>): AuthorizationResult {
    const fn = this.require(target.functionName);
    const policies = [...new Set([target.config.qualifier ?? "", ""])].flatMap(qualifier => {
      const policy = fn.policies?.[qualifier]; if (!policy) return [];
      const document = { Version: "2012-10-17", Statement: policy.statements };
      return [evaluateResourcePolicy(document, principal, action, target.functionArn, context, resourcePolicySource("lambda", `${fn.functionArn}${qualifier ? `:${qualifier}` : ""}`, document, policy.revisionId))];
    });
    const selected = policies.find(policy => policy.decision === "explicitDeny") ?? policies.filter(policy => policy.decision === "allowed").sort((a, b) => ({ wildcard: 0, account: 1, role: 2, directUser: 3, directSession: 3 }[b.grantBasis ?? "wildcard"] - { wildcard: 0, account: 1, role: 2, directUser: 3, directSession: 3 }[a.grantBasis ?? "wildcard"]))[0];
    return { ...(selected ?? { decision: "implicitDeny" as const, reason: "No applicable resource policy Allow statement was found", matchedStatements: [] }), ...mergeProvenance(...policies) };
  ...
  private async invokeZipWorkerRuntime(resolved: ResolvedFunction, event: unknown, requestId: string, options: InvokeOptions, streamCallbacks?: LambdaStreamCallbacks): Promise<InvokeResult> {
    ...
    try {
      ...
      const logLines = this.formatLogLines(executable, fn.functionName, requestId, resolved.executedVersion, durationMs, billedDurationMs, applicationLines, options.serviceLogContext, result.memoryUsage);
      ...
      const common = { memoryUsage: result.memoryUsage ?? { status: "unavailable" as const, reason: "worker-ended-without-measurement" as const }, statusCode: 200, logResult: logBytes.subarray(Math.max(0, logBytes.length - 4096)).toString("base64"), requestId, durationMs, billedDurationMs, executedVersion: resolved.executedVersion };
  ...
  private async invokeImageRuntime(resolved: ResolvedFunction, payload: Buffer, requestId: string, options: InvokeOptions, streamCallbacks?: LambdaStreamCallbacks): Promise<InvokeResult> {
    ...
    try {
      ...
      await this.writeLogs(fn, executable.role, executable.loggingConfig.logGroup, streamName, logLines, [...(options.lineage ?? []), resolved.qualifiedArn]); const logBytes = Buffer.from(logLines.join("\n") + "\n"); const invocation: InvokeResult = { memoryUsage: { status: "unavailable", reason: "container-peak-unavailable" }, payload: result.payload, ...(result.functionError ? { functionError: result.functionError } : {}), statusCode: 200, logResult: logBytes.subarray(Math.max(0, logBytes.length - 4096)).toString("base64"), requestId, durationMs: result.durationMs, billedDurationMs, executedVersion: resolved.executedVersion };
  ...
  private formatLogLines(executable: Executable, functionName: string, requestId: string, version: string, durationMs: number, billedDurationMs: number, application: Array<{ message: string; level: string }>, serviceLogContext?: InvokeOptions["serviceLogContext"], observedMemory?: LambdaMemoryUsage): string[] {
    const memoryUsage: LambdaMemoryUsage = observedMemory ?? { status: "unavailable", reason: executable.packageType === "Image" ? "container-peak-unavailable" : "worker-ended-without-measurement" };
    const memoryText = memoryUsage.status === "measured" ? `${memoryUsage.maxMemoryUsedMB.toFixed(2)} MB (worker lifetime peak RSS; excludes child processes)` : `unavailable (${memoryUsage.reason})`;
    ...
    if (systemEnabled) { if (executable.loggingConfig.logFormat === "JSON") { output.push(JSON.stringify({ time: now, type: "platform.runtimeDone", record: { requestId, status: "success" } })); output.push(JSON.stringify({ time: now, type: "platform.report", record: { requestId, metrics: { durationMs: Number(durationMs.toFixed(2)), billedDurationMs, memorySizeMB: executable.memorySize, ...(memoryUsage.status === "measured" ? { maxMemoryUsedMB: memoryUsage.maxMemoryUsedMB } : {}) }, memoryUsage, resourceLimits: { memory: executable.packageType === "Image" ? "docker-limit-requested" : "descriptor-only", scratch: executable.packageType === "Image" ? "docker-limit-requested" : "descriptor-only" } } })); } else output.push(end, report); }
    return output;
  }
