...
function exact(value: unknown, fields: readonly string[], path: string, issues: ProviderValidationIssue[]): value is Record<string, unknown> {
  ...
  for (const key of Object.keys(value)) if (!fields.includes(key)) issues.push({ code: "UnsupportedProperty", path: `${path}.${key}`, pathSegments: providerValidationPathSegments(`${path}.${key}`), message: `${key} is not part of the frozen AMX-04 helper protocol` });
  return true;
}
function validateAutoDelete(properties: Record<string, unknown>): readonly ProviderValidationIssue[] {
  ...
  if (typeof properties.BucketName !== "string" || !properties.BucketName) issues.push({ code: "InvalidProperty", path: "Properties.BucketName", pathSegments: providerValidationPathSegments("Properties.BucketName"), message: "BucketName must be a nonempty resolved bucket name" });
  return issues;
}
function validateAmplifyTable(properties: Record<string, unknown>): readonly ProviderValidationIssue[] {
  ...
  if (!Array.isArray(definitions) || definitions.length !== 1 || !exact(definitions[0], ["attributeName", "attributeType"], "Properties.attributeDefinitions.0", issues) || definitions[0].attributeName !== "id" || definitions[0].attributeType !== "S") issues.push({ code: "InvalidProperty", path: "Properties.attributeDefinitions", pathSegments: providerValidationPathSegments("Properties.attributeDefinitions"), message: "The frozen helper requires exactly the id/String attribute definition" });
  const keys = properties.keySchema;
  if (!Array.isArray(keys) || keys.length !== 1 || !exact(keys[0], ["attributeName", "keyType"], "Properties.keySchema.0", issues) || keys[0].attributeName !== "id" || keys[0].keyType !== "HASH") issues.push({ code: "InvalidProperty", path: "Properties.keySchema", pathSegments: providerValidationPathSegments("Properties.keySchema"), message: "The frozen helper requires exactly the id/HASH key schema" });
  if (exact(properties.sseSpecification, ["sseEnabled"], "Properties.sseSpecification", issues) && properties.sseSpecification.sseEnabled !== false) issues.push({ code: "InvalidProperty", path: "Properties.sseSpecification.sseEnabled", pathSegments: providerValidationPathSegments("Properties.sseSpecification.sseEnabled"), message: "The frozen helper emits sseEnabled false" });
  if (exact(properties.streamSpecification, ["streamViewType"], "Properties.streamSpecification", issues) && properties.streamSpecification.streamViewType !== "NEW_AND_OLD_IMAGES") issues.push({ code: "InvalidProperty", path: "Properties.streamSpecification.streamViewType", pathSegments: providerValidationPathSegments("Properties.streamSpecification.streamViewType"), message: "The frozen helper emits NEW_AND_OLD_IMAGES" });
  if (properties.provisionedThroughput !== undefined && exact(properties.provisionedThroughput, ["ReadCapacityUnits", "WriteCapacityUnits"], "Properties.provisionedThroughput", issues)) for (const key of ["ReadCapacityUnits", "WriteCapacityUnits"]) if (!Number.isSafeInteger(properties.provisionedThroughput[key]) || Number(properties.provisionedThroughput[key]) < 1) issues.push({ code: "InvalidProperty", path: `Properties.provisionedThroughput.${key}`, pathSegments: providerValidationPathSegments(`Properties.provisionedThroughput.${key}`), message: `${key} must be a positive integer` });
  if (properties.pointInTimeRecoverySpecification !== undefined && exact(properties.pointInTimeRecoverySpecification, ["PointInTimeRecoveryEnabled"], "Properties.pointInTimeRecoverySpecification", issues) && typeof properties.pointInTimeRecoverySpecification.PointInTimeRecoveryEnabled !== "boolean") issues.push({ code: "InvalidProperty", path: "Properties.pointInTimeRecoverySpecification.PointInTimeRecoveryEnabled", pathSegments: providerValidationPathSegments("Properties.pointInTimeRecoverySpecification.PointInTimeRecoveryEnabled"), message: "PointInTimeRecoveryEnabled must be boolean" });
  if (properties.billingMode !== undefined && properties.billingMode !== "PAY_PER_REQUEST" && properties.billingMode !== "PROVISIONED") issues.push({ code: "InvalidProperty", path: "Properties.billingMode", pathSegments: providerValidationPathSegments("Properties.billingMode"), message: "billingMode must be PAY_PER_REQUEST or PROVISIONED" });
  if (typeof properties.tableName !== "string" || !properties.tableName) issues.push({ code: "InvalidProperty", path: "Properties.tableName", pathSegments: providerValidationPathSegments("Properties.tableName"), message: "tableName must be nonempty" });
  return issues;
}
