import { trustPolicySource, resourcePolicySource, authorizationDecision } from "./iam/provenance.js";
...
import type { ApiGatewayTraceScope } from "./xray/collector.js";
...
export class ApiGatewayService {
  ...
  private async ensureTracingRole(enabled = true): Promise<void> { if (enabled) await this.ensureApiGatewayServiceLinkedRole?.(); }
  ...
  async invoke(req: IncomingMessage, res: ServerResponse, pathname: string, url: URL): Promise<void> {
    const started = performance.now(); let apiName: string | undefined; let metricStage: string | undefined; let logApi: RestApiState | undefined; let logStage: ApiStageState | undefined; let logInput: InvocationInput | undefined; let traceScope: ApiGatewayTraceScope | undefined; let metricResource: string | undefined; let metricMethod: string | undefined; let methodSetting: ApiMethodSettingState = {}; let canary = false; const invocationRequestId = id(24);
    try {
      ...
      logInput = { method, path: requestPath, headers, query, multiQuery, pathParameters: routed?.parameters ?? {}, body: Buffer.alloc(0), stageName, stageVariables: selected.variables, requestId: invocationRequestId, deploymentId: selected.deploymentId, isCanaryRequest: canary, sourceIp: req.socket.remoteAddress?.replace(/^::ffff:/, ""), userAgent: headers["user-agent"], domainName: headers.host, principal, principalContext, lambdaLineage: principalContext?.lambdaLineage, identityAuthorization: (req as any).awsIdentityAuthorization, authorizationRequestId: (req as any).awsAuthorizationRequestId };
      if (this.xray) {
        traceScope = this.xray.beginApiGatewayRequest({ accountId: this.store.accountId, region: this.region, apiId: api.id, apiName, stageName, requestId: invocationRequestId, method, url: `${this.invokeProtocol}://${headers.host || `${api.id}.execute-api.${this.region}.amazonaws.com`}/${stageName}${requestPath}${url.search}`, clientIp: logInput.sourceIp, userAgent: logInput.userAgent, incomingHeader: headers["x-amzn-trace-id"], active: stage.tracingEnabled === true });
        logInput.traceScope = traceScope; logInput.xrayTraceId = traceScope.sampled ? traceScope.traceId : undefined;
      }
      ...
      await traceScope?.finish(result.status, result.body.length).catch(() => undefined);
  ...
  private async putGatewayLog(groupName: string, streamName: string, message: string): Promise<void> {
    ...
    if (!role || evaluateTrust(role.assumeRolePolicyDocument, "apigateway.amazonaws.com", "sts:AssumeRole", { "aws:PrincipalServiceName": "apigateway.amazonaws.com" }, trustPolicySource(role)).decision !== "allowed") return;
  ...
  private async pipeline(api: RestApiState, resource: ApiResource, configuredMethod: string, input: InvocationInput, configuration: InvocationConfiguration, authorizers: Record<string, ApiAuthorizerState> = {}, resourcePolicy?: PolicyDocument, cache?: InvocationCacheConfiguration): Promise<PipelineResult> {
    ...
    const resourceAuthorization = resourcePolicy ? evaluateResourcePolicy(this.expandApiPolicy(resourcePolicy, api.id), input.principalContext ?? input.principal ?? "anonymous", "execute-api:Invoke", methodArn, { "aws:SourceIp": input.sourceIp, "aws:PrincipalArn": input.principal, "aws:SourceArn": input.sourceArn, "aws:SourceAccount": input.sourceAccount, "aws:TokenIssueTime": input.principalContext?.issuedAt === undefined ? undefined : new Date(input.principalContext.issuedAt).toISOString(), "aws:SourceIdentity": input.principalContext?.sourceIdentity }, resourcePolicySource("apigateway", `arn:aws:apigateway:${this.region}::/restapis/${api.id}`, resourcePolicy)) : undefined;
    ...
    if (method.authorizationType === "AWS_IAM") { const identityAuthorization = this.authMode !== "enforce" || input.stageName === "test-invoke-stage" ? { decision: "allowed", reason: "IAM policy enforcement is disabled", matchedStatements: [] } as AuthorizationResult : input.identityAuthorization ?? { decision: "implicitDeny", reason: "No authenticated IAM authorization decision is available", matchedStatements: [] }; const combined = resourceAuthorization ? combineIdentityAndResourceAuthorization(identityAuthorization, resourceAuthorization, input.principalContext?.accountId === this.store.accountId ? "sameAccount" : "crossAccount") : identityAuthorization;
      const record = [...this.store.ensureAccount(input.principalContext?.accountId ?? this.store.accountId).iam.authorizationDecisions].reverse().find(item => item.requestId === input.authorizationRequestId && item.action === "execute-api:Invoke" && item.resource === methodArn);
      if (record) { Object.assign(record, authorizationDecision({ ...record, decision: combined.decision, reason: combined.reason }, combined)); await this.store.save().catch(() => undefined); }
    ...
    const integrationUri = String(integration.uri ?? "").replace(/\$\{stageVariables\.([A-Za-z0-9_-]+)\}/g, (_match, name) => input.stageVariables[name] ?? "");
    const lambdaArn = integrationUri.match(/functions\/(arn:[^/]+)\/invocations/)?.[1]; const lambdaName = lambdaArn?.split(":function:")[1]?.split(":")[0];
    input.traceScope?.beginIntegration({ name: lambdaArn ? "Lambda" : integration.type === "AWS" ? integrationUri.includes(":sqs:") ? "SQS" : "DynamoDB" : integration.type.startsWith("HTTP") ? "HTTP" : "Mock", operation: lambdaArn ? "Invoke" : integration.integrationHttpMethod, resourceName: lambdaName, url: integrationUri || undefined });
    try { backend = await this.withTimeout(this.callBackend(api, resource, integration, input, mapped, context, configuration), integration.timeoutInMillis ?? 29_000); input.traceScope?.finishIntegration(backend.status); }
    catch (error) { const status = error instanceof AwsError ? error.status : 500; input.traceScope?.finishIntegration(status); if (error instanceof GatewayFailure) throw error; const aws = error instanceof AwsError ? error : new AwsError("InternalFailure", error instanceof Error ? error.message : String(error), 500); if (aws.code === "AccessDeniedException") throw aws; throw new GatewayFailure(aws.code === "IntegrationFailureException" && /timed out/i.test(aws.message) ? "INTEGRATION_TIMEOUT" : "INTEGRATION_FAILURE", aws.code, aws.message, aws.status); }
  ...
  private validateIntegrationRole(roleArn: string): void { const role = Object.values(this.store.ensureAccount().iam.roles).find(candidate => candidate.arn === roleArn); if (!role || evaluateTrust(role.assumeRolePolicyDocument, "apigateway.amazonaws.com", "sts:AssumeRole", { "aws:PrincipalServiceName": "apigateway.amazonaws.com" }, trustPolicySource(role)).decision !== "allowed") throw new AwsError("BadRequestException", "API Gateway cannot assume the configured integration role"); }
