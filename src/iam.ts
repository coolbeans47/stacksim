...
import { policyDocumentValidationError } from "./iam/policy-validation.js";
...
export function validatePolicyDocument(input: unknown, kind: "identity" | "trust" | "role-inline" = "identity"): PolicyDocument {
  ...
  const validationError = policyDocumentValidationError(document, kind === "trust" ? "trust" : "identity");
  if (validationError) throw new AwsError("MalformedPolicyDocument", validationError, 400);
...
export class IamService {
  ...
  private page<T>(operation: string, input: any, values: T[]): { values: T[]; marker?: string; truncated: boolean } {
    let index = 0; if (input.Marker) { try { index = this.tokens.decode<{ index: number }>(operation, input.Marker).index; } catch { throw new AwsError("InvalidInput", "Invalid marker", 400); } }
    const max = input.MaxItems === undefined ? 100 : Number(input.MaxItems); if (!Number.isInteger(max) || max < 1 || max > 1_000) throw new AwsError("InvalidInput", "MaxItems must be an integer from 1 to 1000", 400);
    const page = values.slice(index, index + max); const next = index + page.length; return { values: page, truncated: next < values.length, ...(next < values.length ? { marker: this.tokens.encode(operation, { index: next }) } : {}) };
  }
  ...
  async CreateRole(input: any): Promise<any> {
    ...
    const path = input.Path ?? "/"; const trust = validatePolicyDocument(input.AssumeRolePolicyDocument, "trust"); const role: IamRoleState = { roleName: input.RoleName, roleId: identifier("AROA"), arn: `arn:aws:iam::${this.store.accountId}:role${path}${input.RoleName}`.replace("role//", "role/"), path, createDate: this.clock.now(), description: input.Description, maxSessionDuration: Number(input.MaxSessionDuration ?? 3600), assumeRolePolicyDocument: trust, assumeRolePolicyCanonical: canonicalPolicyDocument(trust), tags: this.tags(input), attachedPolicyArns: [], inlinePolicies: {}, inlinePolicyCanonicalDocuments: {}, ...(input.PermissionsBoundary === undefined ? {} : { permissionsBoundaryArn: input.PermissionsBoundary }) };
  ...
  async DeletePolicy(input: any): Promise<any> { const policy = this.mutablePolicy(input.PolicyArn); if ([...Object.values(this.iam.roles), ...Object.values(this.iam.users), ...Object.values(this.iam.groups)].some(entity => entity.attachedPolicyArns.includes(policy.arn))) throw new AwsError("DeleteConflict", "Policy is attached to entities", 409); delete this.iam.policies[policy.arn]; await this.store.save(); return {}; }
  async CreatePolicyVersion(input: any): Promise<any> { const policy = this.mutablePolicy(input.PolicyArn); if (Object.keys(policy.versions).length >= 5) throw new AwsError("LimitExceeded", "Policy version limit exceeded", 409); const number = Math.max(0, ...Object.keys(policy.versions).map(key => Number(key.slice(1)))) + 1; const versionId = `v${number}`; if (input.SetAsDefault) { for (const version of Object.values(policy.versions)) version.isDefaultVersion = false; policy.defaultVersionId = versionId; } const document = validatePolicyDocument(input.PolicyDocument); const version = policy.versions[versionId] = { versionId, document, canonicalDocument: canonicalPolicyDocument(document), createDate: this.clock.now(), isDefaultVersion: Boolean(input.SetAsDefault) }; policy.updateDate = this.clock.now(); await this.store.save(); return { PolicyVersion: versionView(version, true) }; }
  ...
  async ListPolicyVersions(input: any): Promise<any> { const policy = this.policy(input.PolicyArn); const values = Object.values(policy.versions).sort((a, b) => b.createDate - a.createDate || Number(b.versionId.slice(1)) - Number(a.versionId.slice(1))); const page = this.page("ListPolicyVersions", input, values); return { Versions: page.values.map(version => versionView(version)), IsTruncated: page.truncated, Marker: page.marker }; }
  async SetDefaultPolicyVersion(input: any): Promise<any> { const policy = this.mutablePolicy(input.PolicyArn); const version = policy.versions[input.VersionId]; if (!version) throw new AwsError("NoSuchEntity", "Policy version was not found", 404); for (const item of Object.values(policy.versions)) item.isDefaultVersion = false; version.isDefaultVersion = true; policy.defaultVersionId = version.versionId; policy.updateDate = this.clock.now(); await this.store.save(); return {}; }
  async DeletePolicyVersion(input: any): Promise<any> { const policy = this.mutablePolicy(input.PolicyArn); const version = policy.versions[input.VersionId]; if (!version) throw new AwsError("NoSuchEntity", "Policy version was not found", 404); if (version.isDefaultVersion) throw new AwsError("DeleteConflict", "Cannot delete the default policy version", 409); delete policy.versions[version.versionId]; await this.store.save(); return {}; }
  ...
  async PutRolePolicy(input: any): Promise<any> {
    ...
    (role.inlinePolicyCanonicalDocuments ??= {})[input.PolicyName] = canonicalPolicyDocument(document);
  ...
  async DeleteRolePolicy(input: any): Promise<any> { const role = this.role(input.RoleName); if (!role.inlinePolicies[input.PolicyName]) throw new AwsError("NoSuchEntity", "Inline policy was not found", 404); delete role.inlinePolicies[input.PolicyName]; delete role.inlinePolicyCanonicalDocuments?.[input.PolicyName]; await this.store.save(); return {}; }
  ...
  async TagPolicy(input: any): Promise<any> { const policy = this.mutablePolicy(input.PolicyArn); Object.assign(policy.tags, this.tags(input)); await this.store.save(); return {}; }
  async UntagPolicy(input: any): Promise<any> { const policy = this.mutablePolicy(input.PolicyArn); for (const key of list<string>(input.TagKeys)) delete policy.tags[key]; await this.store.save(); return {}; }
  async ListPolicyTags(input: any): Promise<any> { const policy = this.policy(input.PolicyArn); const values = Object.entries(policy.tags).sort(([left], [right]) => left.localeCompare(right)).map(([Key, Value]) => ({ Key, Value })); const page = this.page("ListPolicyTags", input, values); return { Tags: page.values, IsTruncated: page.truncated, Marker: page.marker }; }
  ...
  async DeleteUser(input: any): Promise<any> {
    ...
    if (user.permissionsBoundaryArn) throw new AwsError("DeleteConflict", "Cannot delete entity, must delete the permissions boundary first.", 409);
  ...
  async DeleteUserPolicy(input: any): Promise<any> { const user = this.user(input.UserName); if (!user.inlinePolicies[input.PolicyName]) throw new AwsError("NoSuchEntity", "Inline policy was not found", 404); delete user.inlinePolicies[input.PolicyName]; delete user.inlinePolicyCanonicalDocuments?.[input.PolicyName]; await this.store.save(); return {}; }
  ...
  async CreateGroup(input: any): Promise<any> {
    ...
    const path = input.Path ?? "/"; const group: IamGroupState = { groupName: input.GroupName, groupId: identifier("AGPA", 17), arn: `arn:aws:iam::${this.store.accountId}:group${path}${input.GroupName}`.replace("group//", "group/"), path, createDate: this.clock.now(), userNames: [], attachedPolicyArns: [], inlinePolicies: {}, inlinePolicyCanonicalDocuments: {} };
  ...
  async DeleteGroupPolicy(input: any): Promise<any> { const group = this.group(input.GroupName); if (!group.inlinePolicies[input.PolicyName]) throw new AwsError("NoSuchEntity", "Inline policy was not found", 404); delete group.inlinePolicies[input.PolicyName]; delete group.inlinePolicyCanonicalDocuments?.[input.PolicyName]; await this.store.save(); return {}; }
